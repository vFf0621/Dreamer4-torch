...
import gym_multi_car_racing.car_dynamics as car_dynamics
...
class FrictionDetector(contactListener):
    ...
    def _contact(self, contact, begin):
        ...
        if u1 is not None and u2 is not None:
            if "car_id" in u1.__dict__ and "car_id" in u2.__dict__:
                self.env.reward[u1.car_id] -= 5
                self.env.reward[u2.car_id] -= 5
...
class MultiCarRacing(gym.Env, EzPickle):
    ...
    def __init__(self, num_agents, render_mode= None,
        ...
        self.observation_space = spaces.Box(low=-0.5, high=0.5, shape=(3, STATE_H, STATE_W), dtype=np.float32)
    ...
    def step(self, action):
        ...
        done = np.zeros(shape=(self.num_agents)).astype(np.bool)
        if action is not None: # First step without action, called from reset()
            ...
            for car_id, car in enumerate(self.cars): 
                ...
                if self.driving_on_grass[car_id].any() or angle_diff > BACKWARD_THRESHOLD:
                    self.reward[car_id] -= 0.5
                    done[car_id] = True
                if self.reward[car_id] < -1:
                    done[car_id] = 1
        ...
        if len(self.track) in self.tile_visited_count:
            done = np.ones(shape=(self.num_agents)).astype(np.bool)
    ...
    def _render_window(self, car_id, mode):
        ...
        for id, car in enumerate(self.cars):
            if self.use_ego_color:  # Apply same ego car color coloring scheme
                car.hull.color = (0.8, 0.0, 0)  # Set all other car colors to blue
        ...
        if mode == "human":
            ...
            return x.transpose(2, 0, 1).astype(np.float32)/255
        else:
            return self.isopen
    ...
    def _render_indicators(self, id, W, H):
        ...
        def render_if_min(value, points, color):
            if abs(value) > 1e-4:
                pygame.draw.polygon(self.surf[id], points=np.array(points), color=color)
    ...
    def _render_road(self,car_id, zoom, translation, angle):
        ...
        for i in range(self.num_agents):
            self._draw_colored_polygon(
             self.surf[car_id], field, self.bg_color, zoom, translation, angle, clip=False
             )
        ...
        for poly in grass:
             self._draw_colored_polygon(
                 self.surf[car_id], poly, self.grass_color, zoom, translation, angle
             )
        ...
        for poly, color in self.road_poly:
             ...
             self._draw_colored_polygon(self.surf[car_id], poly, color, zoom, translation, angle)
