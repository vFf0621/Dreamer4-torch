...
The agent token is injected once, at the first timestep, rather than restamped at every step. Later positions start empty on that channel and pick the token up indirectly: the latents at `t` read `h_{t-1}`, so the token travels forward through the z stream rather than along the readout channel. Note this makes the readout channel a carry within whatever window is fed: `action_step` trims its buffer to `eval_context_len`, so at inference the token is re-seeded at the start of each window.
...
The readout skips the temporal blocks entirely, the way the signal token does. It is built by the spatial path and reaches later steps only because the latents read `h_{t-1}`. That keeps it cheap in parameters -- the temporal blocks hold per-channel weights, so a readout stream of its own width there would have cost as much as the latent stream. With it excluded, the parameter count is flat in `h_tokens` (only the task embedding scales), though the tokens still widen every timestep and so still cost activations.
