...
Action embeddings are interleaved with the latent temporally, not added, as in previous implementations. Each timestep contributes the block `a, z, (t, d)` — action tokens, then latent tokens, then the shortcut signal token — forming the stream `a_1, z_1, (t,d)_1, a_2, z_2, (t,d)_2, ...`. In the dynamics model the temporal layers attend over this flattened stream under a **standard causal mask**, so ordering is strict at token granularity: `z_t` attends the actions of its own timestep and everything before, but not the `(t, d)` token that follows it. The spatial layers stay bidirectional within a timestep, which is what lets `z` read its own shortcut signal token. The tokenizer keeps per-channel independent temporal attention. In the dynamics blocks each modality (`a`, `z`, `(t, d)`, registers) has its own independent feed-forward network, deeper than the tokenizer's shared one.
