...
The readout is a state, not just a probe. Each timestep carries `h_tokens` readout tokens (defaulting to `latent_tokens`, one per latent), and the latents at `t` attend to the readout tokens from `t-1` alongside their own stream and the actions, so the next latents are produced from the previous `h`. The lag is what keeps it causal: nothing reads `h_t` at `t`. The policy consumes the mean over the `h` tokens at the step it acts on.

Note the cost, since `h_tokens` widens every timestep and the temporal blocks carry per-channel weights. At 64 latent tokens, batch 16 and length 60: `h_tokens=1` is 61 GB of activations with 1.42 GB of parameters, `h_tokens=8` is 67 GB and 1.53 GB, and `h_tokens=64` -- the default -- is 114 GB and 2.46 GB. Setting it equal to `latent_tokens` roughly doubles the model, so set it explicitly if you are near a memory limit.
