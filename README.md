...
Action inputs are expected to be normalized in [-1, 1]. Discrete action inputs have not been implemented. Instead, one can set action dim to 1 and set the action bin size to the number of discrete actions. This is equivalent to discrete embedding lookup as actions are converted into onehot vectors anyways.
