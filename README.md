...
That leaves the act-now head trained at one timestep per sequence, so `Dreamer4.bc_act_now_aux` samples `bc_aux_steps` further timesteps and rebuilds each the way `action_step` does: a window of at most `eval_context_len + 1` frames ending at `t`, the executed actions inside it, and the query at `t`. Those passes run under `no_grad` and the features are detached, so they train the policy head only — the cost is a handful of short forward passes, not activation memory.
